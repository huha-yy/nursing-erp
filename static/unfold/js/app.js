/* 覆写自 unfold 0.104.1 static/unfold/js/app.js，仅改侧栏开合状态逻辑，升级 unfold 需重新对照。
 *
 * 原版问题（iPad ≤1280px 视口）：sidebarOpen() 无条件返回 false，且 x-resize 监听在
 * 每次 window resize 时强制收起——iOS Safari 滚动导致工具栏收放同样触发 resize，
 * 于是 iPad 上点开侧栏后随手一滚就被拍回，表现为「侧栏打不开/一直被迫收起」。
 *
 * 本版语义：开/关是用户选择，存 localStorage 跨页面保持；无记录时 >1280 默认开、
 * ≤1280 默认收。resize（含 iOS 工具栏收放）只跟随用户选择，不再强制收起。 */
...
function theme(defaultTheme = "auto") {
	return {
		...
		themeBindings: {
			["x-resize.window"]() {
				// 只跟随用户选择（无记录时按宽度给默认值），不强制收起——
				// iOS Safari 滚动时工具栏收放会高频触发 resize
				const sidebarOpenValue = localStorage.getItem("sidebarOpen");

				if (sidebarOpenValue !== null) {
					this.sidebarOpen = sidebarOpenValue !== "0";
				} else {
