...
class MealOrder(models.Model):
    ...
    def modify_menu(self, new_choice: str, reason: str = ""):
        old = self.menu_choice
        self.menu_choice = new_choice
        self.status = self.Status.MODIFIED
        self.save()
        log_reason = f"原: {old} → 新: {new_choice}"
        if reason:
            log_reason += f" ({reason})"
        MealModificationLog.objects.create(order=self, action="modify", reason=log_reason)
